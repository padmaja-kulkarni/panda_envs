...
class PandaTestEnv(panda_env.PandaEnv, utils.EzPickle):
    def __init__(self):
        ...
        LoadYamlFileParamsTest(rospackage_name="openai_ros",
                               rel_path_from_package_to_file="src/openai_ros/task_envs/panda/config",
                               yaml_file_name="panda_test.yaml")
        rospy.logdebug("Entered PandaTestEnv Env")
        self.get_params()
        
        #panda_env.PandaEnv.__init__(self)


        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(self.n_actions,), dtype=np.float32) #padmaja might need to change it to cont.
    ...
    def _set_init_pose(self):
        ...
        # INIT POSE
        print("Moving To TEST DESIRED GOAL Position ", self.desired_position)
        print("Moving To TEST DESIRED self.gripper_rotation Position ",self.gripper_rotation)
    ...
    def _set_action(self, action):
        ...
        delta_gripper_target[0] +=  action[0] * 0.01
        delta_gripper_target[1] +=  action[1] * 0.01
        delta_gripper_target[2] +=  action[2] * 0.01
